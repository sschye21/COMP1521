...
#
#
# This program was written by STEVEN WEN REI CHYE (z5257742)
# on 14/07/2021
#
...
loop1:
    	...
    	move $a0, $s0
    	move $a1, $v0       
    	beq $a1, 1, loop1    	# while(update_snake(direction))

print_over:
    	lw $t2, snake_body_len  
    	div $t2, $t2, 3         # snake_body_len = snake_body_len / 3
    	move $t3, $t2           # score = snake_body_len / 3

	    la $a0, main__game_over
    	li $v0, 4               # printf("Game over! Your score was ")
    	syscall
...
update_apple:
    ...
    #
    # Frame:    $ra, $s0, $s1
    # Uses:     $v0, $a0, $a1, $a2, $a3, $t0, $t2, $t3
    # Clobbers: $v0, $t0, $t2, $t3
    #
    # Locals:
    #   - 'int apple_row' in $s0
    #	- 'int apple_col' in $s1
...
update_apple__body:
	...
	li $t2, EMPTY		# sets $t2 = EMPTY
	li $t0, N_COLS		# sets $t0 = N_COLS
	
	mul $t0, $t0, $s0       # N_COLS * apple_row
	add $t0, $t0, $s1       # N_COLS * apple_row) + apple_col
	lb  $a3, grid($t0)      # $a3 = grid[apple_row][apple_col]
	beq $a3, $t2, continue  # while (grid[apple_row][apple_col] == EMPTY);
	j update_apple__body

continue:
	li $t3, APPLE 		# sets $t3 = APPLE
	li $t0, N_COLS		# resets $t0 to N_COLS
		
	mul $t0, $t0, $s0       # N_COLS * apple_row
	add $t0, $t0, $s1       # N_COLS * apple_row) + apple_col
	sb $t3, grid($t0)       # grid[apple_row][apple_col] = APPLE;
...
update_snake:
    ...
    #
    # Frame:    $ra, $s0, $s1, $s2
    # Uses:     $v0, $t0, $t1, $t2, $t3, $t4, $t5, $t6, $t7, $a0, $a1, $a2, $a3, 
    # Clobbers: $v0, $t0, $t1, $t2, $t3, $t4, $t5, $t6, $t7, $a0, $a1
    #
    # Locals:
    #   - 'int d_row' in $t0
    #	- 'int d_col' in $s1
...
update_snake__body:
	jal get_d_row
	move $t0, $v0               	# int d_row = get_d_row(direction);
	...
	li $t2, 0
	lb $a1, snake_body_row($t2) 	# int head_row = snake_body_row[0];
	lb $a2, snake_body_col($t2) 	# int head_col = snake_body_col[0];

	li $t3, N_COLS
	li $t4, SNAKE_BODY
	mul $t3, $t3, $a1           
	add $t3, $t3, $a2           
	sb $t4, grid($t3)          	# grid[head_row][head_col] = SNAKE_BODY;

	add $s0, $a1, $t0          	# int new_head_row = head_row + d_row;
	add $s1, $a2, $s1          	# int new_head_col = head_col + d_col;

	li $t5, N_ROWS
	li $t3, N_COLS
	blt $s0, 0, loadingfalse	# if (new_head_row < 0)       return false;
	bge $s0, $t5, loadingfalse      # if (new_head_row >= N_ROWS) return false;
	blt $s1, 0, loadingfalse        # if (new_head_col < 0)       return false;
	bge $s1, $t3, loadingfalse      # if (new_head_col >= N_COLS) return false;

	mul $t3, $t3, $s0               # N_COLS * new_head_row
	add $t3, $t3, $s1               # (N_COLS * new_head_row) + new_head_col
	lb  $a1, grid($t3)              # $a1 -> grid[new_head_row][new_head_col];
	
	li $t6, APPLE
	bne $a1, $t6, setfalse
	li $a2, 1                       # bool apple = true
	j updatingsnake

setfalse:
	li $a2, 0                       # bool apple = false
	j updatingsnake

updatingsnake:
	lw $t6, snake_body_len          
	addi $t6, $t6, -1               # snake_body_len - 1
	
	lw $a3, snake_tail
	move $a3, $t6                   # snake_tail = snake_body_len - 1
	sw $a3, snake_tail		# store value of $a3 into snake_tail
	...
	move $t7, $v0			# moves the returned result into $t7
	
	beq $t7, 0, loadingfalse        # if (! move_snake_in_grid(new_head_row, new_head_col))

	jal move_snake_in_array

	move $a2, $s2
	bne $a2, 1, loadtrue            # if (!apple) goto loadtrue
	lw $t7, snake_growth            
	addi $t7, $t7, 3                # snake_growth += 3;
	sw $t7, snake_growth
...
move_snake_in_grid:
    ...
    #
    # Frame:    $ra, $s0, $s1
    # Uses:     $v0, $t0, $t1, $t2, $t3, $t4, $t5, $t6, $t7, $a0, $a1
    # Clobbers: $v0, $t0, $t1, $t2, $t3, $t4, $t5, $t6, $t7
    #
    # Locals:
    #   - 'int tail' in $t3
    #	- 'int_tail_row' in $t4
...
move_snake_in_grid__body:
	...
	lw $t0, snake_growth
	ble $t0, 0, elses           	# if (snake_growth > 0) {
...
elses:
	...
	lb $t4, snake_body_row($t3) 	# tail_row = snake_body_row[tail];
	lb $t5, snake_body_col($t3) 	# tail_col = snake_body_col[tail];

	li $t6, N_COLS                  
	li $t2, EMPTY
	mul $t6, $t6, $t4               # N_COLS * tail_row
	add $t6, $t6, $t5               # (N_COLS * tail_row) + tail_col
	sb  $t2, grid($t6)              # grid[tail_row][tail_col] = EMPTY;
...
nextif:
	...
	mul $s0, $s0, $a0
	add $s0, $s0, $a1
	lb $s1, grid($s0)           	# $s1 = grid[new_head_row][new_head_col]
	beq $s1, $t7, loadfalse

continuing:
	li $t3, SNAKE_HEAD
	li $s0, N_COLS                  # setting $s0 to N_COLS
	mul $s0, $s0, $a0               # N_COLS * new_head_row
	add $s0, $s0, $a1               # (N_COLS * new_head_row) + new_head_col
	sb $t3, grid($s0)               # grid[new_head_row][new_head_col] = SNAKE_HEAD; <- HERE FAILS TOO 
...
move_snake_in_array__body:
	move $s0, $a0			# move $a0 into saved register
	move $s1, $a1			# move $a1 into saved register

	lw $a2, snake_tail          	# $t0 = snake_tail = i

loop:
	blt $a2, 1, continued       	# if (i < 1) goto continued
