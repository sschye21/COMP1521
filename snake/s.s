...
loop1:
    ...
    move $t1, $v0       
    beq $t1, 1, loop1    # while(update_snake(direction))

print_over:
    lw $a0, snake_body_len  
    div $a0, $a0, 3         # snake_body_len = snake_body_len / 3
    move $t2, $a0           # score = snake_body_len / 3
...
update_apple__body:
    li $a0, N_ROWS 
    jal rand_value
    move $a1, $v0       # apple_row = rand_value(N_ROWS);

    li $a0, N_COLS
    jal rand_value
    move $a2, $v0       # apple_col = rand_value(N_COLS);

    li $t2, EMPTY
    li $t0, N_COLS

    mul $t0, $t0, $a1       # N_COLS * apple_row
    add $t0, $t0, $a2       # N_COLS * apple_row) + apple_col
...
update_snake__body:
    ...
    jal get_d_col
    move $t1, $v0               # int d_col = get_d_col(direction);
    ...
    add $s0, $t0, $a1           # int new_head_row = head_row + d_row;
    add $s1, $t1, $a2           # int new_head_col = head_col + d_col;

    li $t5, N_ROWS
    li $t3, N_COLS
    blt $s0, 0, returnfalse             # if (new_head_row < 0)       return false;
    bge $s0, $t5, returnfalse           # if (new_head_row >= N_ROWS) return false;
    blt $s1, 0, returnfalse             # if (new_head_col < 0)       return false;
    bge $s1, $t3, returnfalse           # if (new_head_col >= N_COLS) return false;
    ...
    #move $a0, $a1       # $A1 PRINTS 2 UGH
    #li $v0, 1
    #syscall
    
    bne $a1, $t6, setfalse
    li $a2, 1                       # bool apple = true -> $a2
    j updatingsnake
...
updatingsnake:
    lw $a3, snake_body_len          
    addi $a3, $a3, -1               # snake_body_len - 1
    lw $s3, snake_tail
    move $s3, $a3              # snake_tail = snake_body_len - 1
    ...
    move $t7, $v0
    beq $t7, 0, update_snake__epilogue

    jal move_snake_in_array

    bne $a2, 1, update_snake__epilogue
    lw $t7, snake_growth
    addi $t7, $t7, 3
    sw $t7, snake_growth
    jal update_apple
...
elses:
    ...
    li $t6, N_COLS                  # setting $t0 to N_COLS
    li $t3, EMPTY
    mul $t6, $t6, $t4               # N_COLS * tail_row
    add $t6, $t6, $t5               # (N_COLS * tail_row) + tail_col
    sb  $t3, grid($t6)              # grid[tail_row][tail_col] = EMPTY;

nextif:
    li $a3, SNAKE_BODY
    beq $s1, $a3, returningfalse

continuing:
    li $t3, SNAKE_HEAD
    li $s0, N_COLS                  # setting $t0 to N_COLS
    mul $s0, $s0, $a0               # N_COLS * new_head_row
    add $s0, $s0, $a1               # (N_COLS * new_head_row) + new_head_col
    sb $s0, grid($t6)               # grid[new_head_row][new_head_col] = SNAKE_HEAD; <- HERE FAILS TOO 
...
move_snake_in_array__prologue:
    ...
    addiu   $sp, $sp, -12
    sw  $ra, 8($sp)
    sw  $s0, 4($sp)             # int new_head_row
    sw  $s1,  ($sp)             # int new_head_col

move_snake_in_array__body:
    move $s0, $a0
    move $s1, $a1

    lw $t0, snake_tail          # $t0 = snake_tail = i

loop:
    blt $t0, 1, continued       # if (i < 1) goto continued
